...
```{r deeplearning_gbm_drf_glm, results="hide"}
...
```{r deeplearning_evolution_training_time, results="hide"}
#dev.new(noRStudioGD=FALSE) #direct plotting output to a new window
par(mfrow=c(2,2)) #set up the canvas for 2x2 plots
ep <- c(1,250,500,750)
...
```{r deeplearning_for_loop_hidden, results="hide"}
#dev.new(noRStudioGD=FALSE) #direct plotting output to a new window
par(mfrow=c(2,2)) #set up the canvas for 2x2 plots
...
```{r compare_activation_functions, results="hide"}
#dev.new(noRStudioGD=FALSE) #direct plotting output to a new window
par(mfrow=c(2,2)) #set up the canvas for 2x2 plots
...
```{r deeplearning_ctype_one_epoch}
m1 <- h2o.deeplearning(
...
```{r random_hyper_params, results="hide"}
hyper_params <- list(
...
```{r binary_classification, results="hide"}
dlmodel <- h2o.deeplearning(
  x=predictors,
  y="bin_response", 
  training_frame=train,
...
```{r bin_class_as_factor, results="hide"}
